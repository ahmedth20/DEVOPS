...
//addAndAssignEtudiantToEquipeAndContract(Etudiant e, Integer idContrat, Integer idEquipe)
	...
	@GetMapping("/filter")
	public List<Etudiant> filterEtudiants(
			@RequestParam(required = false) String nomE,
			@RequestParam(required = false) String prenomE,
			@RequestParam(required = false) Option op) {
		return etudiantService.filterEtudiants(nomE, prenomE, op);
	}

	@GetMapping("/sorted")
	public List<Etudiant> getAllEtudiantsSortedByNom() {
		return etudiantService.getAllEtudiantsSortedByNom();
	}
